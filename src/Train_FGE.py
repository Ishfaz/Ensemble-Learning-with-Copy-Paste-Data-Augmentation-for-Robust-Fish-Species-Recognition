...
def main(data_path: str, model_path: str, save_dir: str, epochs: int, base_epoch: int, batch_size: int, image_size: int, base_lr: float=0.0001, max_lr: float=0.01):
    ...
    ### Save the final FGE model
    final_fge_path = f"{save_dir}/final_fge_model.pt"
    model.save(final_fge_path)
    print(f"\nFinal FGE model saved at {final_fge_path}\n")
    
    print("\n Training Completed!!!\n")
    
if __name__ == "__main__":
    ### Taking input arguments from the user
    parser = argparse.ArgumentParser(description="Train YOLOv8 with custom augmentations and FGE")
    ...
    parser.add_argument("--base_epoch", type=int, default=260, help="Base epoch for FGE")
